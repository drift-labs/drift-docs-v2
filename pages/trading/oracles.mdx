...
For robustness, Drift's program checks oracle validity. The validity is evaluated on a per-check and action basis to determine whether to block actions. See [**Protocol Guard Rails**](pages/risk-and-safety/protocol-guard-rails.mdx) for more details.

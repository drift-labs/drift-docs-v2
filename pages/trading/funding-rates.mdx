...
Note: Funding rate magnitudes are clamped at 0.12626% and are delayed at large divergences (see [**Oracles**](pages/trading/oracles.mdx)). Individual Market TWAP updates utilize the side of the book for trade executions, Bid and Ask TWAPs in the market are calculated and/or estimated on every trade.
...
The cumulative funding rate is checked against user positions in case the off-chain funding rate bot does not trigger on the hour. This will show up as "Unrealised P&L" until your next user action within the market (such as a trade, deposit, withdraw etc.), see also [**P&L**](pages/profit-loss/profit-loss-intro.mdx).
...
See [**Rebates**](pages/market-makers/maker-rebate-fees.mdx) for funding pool sizes where the funding paid from the Rebate Pool will be capped. This ensures that the risk is spread in the system and the Insurance Fund isn't drained by funding rates.
...
![example data](public/assets/mBFRK1HfhF-9o6IaExcW0_screen-shot-2021-12-30-at-115308-am.png)
...
Read more about protective checks in [**Oracles**](pages/trading/oracles.mdx).

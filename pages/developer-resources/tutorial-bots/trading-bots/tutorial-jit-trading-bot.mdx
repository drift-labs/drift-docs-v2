...
JIT Maker Bots maintain a local copy of the decentralised orderbook (DLOB) (see: [**Keepers & Decentralised Orderbook**](/about-v2/keepers-decentralized-orderbook-faq)) in order to see which orders are in the auction phase and can be bid on.
...
Follow the instructions at [**Keeper Bots**](/developer-resources/tutorial-bots/keeper-bots) to set the required environment variables, initialise the`ClearingHouseUser` and deposit some collateral.
...
Technical details on the JIT Auction and its pricing can be found at [**Just-In-Time (JIT) Auctions**](/about-v2/jit-auctions). The reference implementation acts on each user order received and makes the order in the opposite direction at the auction end price with a random order size between `20` and `MAX_TRADE_SIZE_QUOTE`.

...
Further risk parameters are also set out in  and [**Risk Parameters**](pages/risk-and-safety/risk-parameters.mdx).
...
The DAMM has protections in place to prevent the long short imbalance from skewing too heavily to either side. More information on that is here at: [**Drift AMM**](pages/about-v2/drift-amm.mdx). Despite these protections, there is still a risk that the DAMM will be exposed to delta risk in periods of significant volatility.
...
These rules are systemitised and can be read more in [**P&L**](pages/profit-loss/profit-loss-intro.mdx) and [**Margin**](pages/trading/margin.mdx)

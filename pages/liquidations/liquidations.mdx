...
If a position drops below its Minimum Maintenance Margin, liquidators are incentivized by the protocol to take over positions so a user's remaining collateral may settle the losses that have occurred. Read [**Insurance Fund**](pages/insurance-fund/insurance-fund-intro.mdx) for what happens when prices move rapidly and/or liquidations don't happen in time.  
...
Note that the Minimum Maintenance Margin for a SOL-PERP is 5% (see Minimum Maintenance Margin for assets here: [**Margin**](pages/trading/margin.mdx))
...
Read [**Liquidators**](pages/liquidations/liquidators.mdx) if you want to learn more about being a liquidator on the platform. Liquidators earn a fee for their services.  
